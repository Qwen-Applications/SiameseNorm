...
from . import LayerNormBase, LayerNorm, RMSLayerNorm
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.c_norm: Optional[LayerNormBase] = None
        if config.use_query_norm or config.use_key_norm:
            assert not config.attention_layer_norm, "with use_query_norm or use_key_norm set, attention_layer_norm must be False"
        ...
        if config.use_query_norm:
            self.q_norm = LayerNormBase.build(
                config, 
                size=(config.d_model // config.n_heads),
                elementwise_affine=config.attention_layer_norm_with_affine,
            )
        if config.use_key_norm:
            self.k_norm = LayerNormBase.build(
                config, 
                size=(config.d_model // config.n_heads),
                elementwise_affine=config.attention_layer_norm_with_affine,
        ...
        if config.use_context_norm:
            self.c_norm = LayerNormBase.build(
                config, 
                size=(config.d_model // config.n_heads),
                elementwise_affine=config.attention_layer_norm_with_affine,
            )

        self.layer_norm_pos_configs = {
            i: {'attn_norm': self.config.attn_norm, 'attn_residual': self.config.attn_residual_after_norm,
                'ffn_norm': self.config.ffn_norm, 'ffn_residual': self.config.ffn_residual_after_norm} 
            for i in range(self.config.n_layers)
        }
        if self.config.norm_pos_special_config:
            #  "_{(0,1):{'attn_norm':False,'attn_residual':True}}_"
            cur_norm_pos_special_config = eval(self.config.norm_pos_special_config.strip('_'))
            for k, v in cur_norm_pos_special_config.items():
                if isinstance(k, int):
                    k = [k]
                for _cur_layer_id in k:
                    self.layer_norm_pos_configs[_cur_layer_id].update(v)
        if int(os.environ.get('RANK', '0')) == 0 and self.layer_id == 0:
            for k, v in self.layer_norm_pos_configs.items():
                log.info(f'{"="*30} norm_pos_info[{k}] = {v} {"="*30}')
    ...
    def reset_parameters(self):
        ...
        if self.v_norm is not None:
            self.v_norm.reset_parameters()
        if self.c_norm is not None:
            self.c_norm.reset_parameters()
    ...
    def attention(
        ...
        if self.c_norm is not None:
            att = self.c_norm(att).to(dtype=dtype)
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.layer_id = layer_id
        # Layer norms.
        self.attn_norm = LayerNorm.build(config, size=config.d_model) if self.layer_norm_pos_configs[self.layer_id]['attn_norm'] else None
        self.c1 = nn.Parameter(torch.ones(config.d_model).to(torch.float32)) if not self.layer_norm_pos_configs[self.layer_id]['attn_norm'] else 1.0
        self.ff_norm = LayerNorm.build(config, size=config.d_model) if self.layer_norm_pos_configs[self.layer_id]['ffn_norm'] else None
        self.attn_y_norm = LayerNorm.build(config, size=config.d_model) 
        self.ff_y_norm = LayerNorm.build(config, size=config.d_model)
        self.ff_input_norm = LayerNorm.build(config, size=config.d_model)
    def reset_parameters(self):
        ...
        if self.attn_y_norm is not None:
            self.attn_y_norm.reset_parameters()
        if self.ff_y_norm is not None:
            self.ff_y_norm.reset_parameters()
        if self.ff_input_norm is not None:
            self.ff_input_norm.reset_parameters()
        self.c1.data.fill_(1.0)
    ...
    def forward(
        ...
        # apply norm before
        normed_y = self.attn_y_norm(y)
        if self.layer_norm_pos_configs[self.layer_id]['attn_norm']:
            #raise NotImplementedError "no attn norm"
            if self.layer_norm_pos_configs[self.layer_id]['attn_residual']:
                raise NotImplementedError("no attn residual after norm")
                if self._activation_checkpoint_fn is not None:
                    x = self._activation_checkpoint_fn(self.attn_norm, x) + normed_y
                else:
                    x = self.attn_norm(x)+normed_y
                h = x
            else:
                if self._activation_checkpoint_fn is not None:
                    h = self._activation_checkpoint_fn(self.attn_norm, x) + normed_y
                else:
                    h = self.attn_norm(x) + normed_y
        else:
            h = x * self.c1 + normed_y
        ...
        # shape: (B, T, C)
        res = self.dropout(att)
        c = ((self.layer_id+1)*2)**0.5
        #c = 1
        ...
        if self.layer_norm_pos_configs[self.layer_id]['ffn_norm']:
            if not self.layer_norm_pos_configs[self.layer_id]['ffn_residual']:
                og_x = x
            if self._activation_checkpoint_fn is not None:
                x = self._activation_checkpoint_fn(self.ff_norm, x)  # type: ignore
            else:
                x = self.ff_norm(x)
            if self.layer_norm_pos_configs[self.layer_id]['ffn_residual']:
                og_x = x
        else:
            og_x = x

        normed_y = self.ff_y_norm(y)
        x = self.ff_proj(self.ff_input_norm(x+normed_y))
...
class OLMo(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                ln_y_f=LayerNorm.build(config),
                ln_final_f=LayerNorm.build(config),
            )
        )
    ...
    def reset_parameters(self):
        ...
        self.transformer.ln_y_f.reset_parameters()  # type: ignore
        self.transformer.ln_final_f.reset_parameters()  # type: ignore
        # Output weights.
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            ...
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if should_checkpoint_block(self.activation_checkpointing_strategy, block_idx):
                    # shape: (batch_size, seq_len, d_model)
                    x, y, cache = self._activation_checkpoint_fn(
                        block,
                        x, y,
                        attention_bias=attention_bias,
                        layer_past=layer_past,
        ...
        y = self.transformer.ln_y_f(y)  # type: ignore
        x = self.transformer.ln_final_f(x + y)  # type: ignore
        if output_hidden_states:
